...
def make_premium_button(text: str, callback_data: str, emoji_id: str = None):
    """Создает кнопку с Telegram Premium эмодзи"""
    if emoji_id:
        return InlineKeyboardButton(text=f'<tg-emoji emoji-id="{emoji_id}"> </tg-emoji>{text}', callback_data=callback_data)
...
def main_menu_keyboard(is_admin=False):
    ...
    if is_admin:
        buttons.append([make_premium_button("Админ-панель", "menu_admin", None)])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def admin_panel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [make_premium_button("Выдать ключ пользователю", "admin_give_key", None)],
...
<tg-emoji emoji-id="5985346521103604145"> </tg-emoji> <b>1. Возврат:</b> Возврата нет — при покупке цифрового товара потеря, неправильное использование никто не компенсирует.

<tg-emoji emoji-id="5985346521103604145"> </tg-emoji> <b>2. Ответственность:</b> Исполнитель не несёт ответственности за последствия применения.

<tg-emoji emoji-id="5985346521103604145"> </tg-emoji> <b>3. Общие:</b> Оплачивая услугу, вы соглашаетесь с данными правилами.

<tg-emoji emoji-id="5985346521103604145"> </tg-emoji> <b>4. Заключительные:</b> Исполнитель вправе изменять условия.

<tg-emoji emoji-id="5985346521103604145"> </tg-emoji> Нажмите на кнопку ниже, чтобы продолжить"""
...
async def menu_purchases(callback: types.CallbackQuery):
    ...
    if not purchases:
        ...
        text = f"""<tg-emoji emoji-id="5983399041197675256"> </tg-emoji> <b>ИСТОРИЯ ПОКУПОК:</b>\n\n"""
...
async def menu_admin(callback: types.CallbackQuery):
    ...
    await callback.message.edit_caption(caption=text, reply_markup=admin_panel_keyboard())
    await callback.answer()
...
async def admin_give_key(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_caption(caption=text, reply_markup=cancel_keyboard())
...
async def admin_confirm_uah(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_caption(caption="📋 <b>Выберите UAH оплату для подтверждения:</b>", reply_markup=builder.as_markup())
    await callback.answer()
...
async def admin_confirm_crypto(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_caption(caption="📋 <b>Выберите CRYPTO оплату для подтверждения:</b>", reply_markup=builder.as_markup())
    await callback.answer()
